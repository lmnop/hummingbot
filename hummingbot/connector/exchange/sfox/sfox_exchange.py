...
from hummingbot.connector.exchange.sfox.sfox_order_book_tracker import SfoxOrderBookTracker
...
from hummingbot.connector.exchange.sfox.sfox_in_flight_order import SfoxInFlightOrder
...
class SfoxExchange(ExchangeBase):
    """
    SfoxExchange connects with SFOX exchange and provides order book pricing, user account tracking and
    ...
    def __init__(self,
        ...
        """
        :param sfox_api_key: The API key to connect to private SFOX APIs.
        ...
        self._sfox_auth = SfoxAuth(sfox_api_key)
        self._order_book_tracker = SfoxOrderBookTracker(throttler=self._throttler,
                                                        trading_pairs=trading_pairs)
        self._user_stream_tracker = SfoxUserStreamTracker(throttler=self._throttler,
        ...
        self._in_flight_orders = {}  # Dict[client_order_id:str, SfoxInFlightOrder]
        ...
        self._update_order_status_fetching = False
    ...
    @property
    def in_flight_orders(self) -> Dict[str, SfoxInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        ...
        self._in_flight_orders.update({
            key: SfoxInFlightOrder.from_json(value)
    ...
    async def _update_trading_rules(self):
        ...
        self._trading_rules = self._format_trading_rules(symbols_info.get("data", []))

    def _format_trading_rules(self, symbols_info: Dict[str, Any]) -> Dict[str, TradingRule]:
        """
        Converts json API response into a dictionary of trading rules (hardcoded!).
        ...
        for rule in symbols_info:
            try:
                trading_pair = convert_from_exchange_trading_pair(rule["pair"])
                is_fiat_denom = is_fiat_quote_pair(trading_pair)
                result[trading_pair] = TradingRule(trading_pair,
                                                   # min_order_size=Decimal("0.00000001") if is_fiat_denom else Decimal("0.001"),
                                                   min_order_size=Decimal("0.001"),
                                                   min_price_increment=Decimal("0.01") if is_fiat_denom else Decimal("0.00000001"),
                                                   min_base_amount_increment=Decimal("0.00000001"),
                                                   min_notional_size=Decimal("5") if is_fiat_denom else s_decimal_0,
                                                   # min_order_value=Decimal("5") if is_fiat_denom else s_decimal_0,
                                                   )
            except Exception:
    ...
    async def _api_request(self,
        ...
        async with self._throttler.execute_task(_limit_id):
            ...
            if is_auth_required:
                headers: dict = self._sfox_auth.get_headers(method, f"{Constants.REST_URL_AUTH}/{endpoint}",
    ...
    async def _create_order(self,
        ...
        api_params = {"client_order_id": order_id,
                      ...
                      "algorithm_id": 100 if order_type == OrderType.MARKET else 201,
        ...
        try:
            order_result = await self._api_request("POST",
                                                   Constants.ENDPOINT["ORDER_CREATE"].format(side=trade_type.name.lower()),
                                                   params=api_params,
                                                   is_auth_required=True,
                                                   limit_id=Constants.RL_ID_ORDER_CREATE)
            order_state = order_result.get('status', "").lower()
            if order_state in {"canceled", "rejected", "cancel pending"}:
                raise SfoxAPIError({'error_code': 'ORDER_REJECTED', 'error': 'Order rejected.'})
            if order_state != 'started':
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = SfoxInFlightOrder(
    ...
    async def _execute_cancel(self, trading_pair: str, order_id: str) -> str:
        ...
        :param trading_pair: The market trading pair (Unused during cancel on SFOX)
        ...
        try:
            ...
            order_result = await self._api_request("DELETE",
                                                   Constants.ENDPOINT["ORDER_DELETE"].format(id=ex_order_id),
                                                   is_auth_required=True,
                                                   limit_id=Constants.RL_ID_ORDER_DELETE)
            if order_result.get('status', '').lower() == "cancel pending":
                order_was_cancelled = True
            else:
                self.logger().network(f"Unexpected order result:\n{order_result}")
        except asyncio.CancelledError:
            raise
        except (asyncio.TimeoutError, SfoxAPIError) as e:
            ...
            if err_code == 'ORDER_NOT_FOUND' and \
    ...
    async def _update_balances(self):
        ...
        """
        last_tick = int(self._last_poll_timestamp / Constants.UPDATE_BALANCE_INTERVAL)
        current_tick = (0 if math.isnan(self.current_timestamp)
                        else int(self.current_timestamp / Constants.UPDATE_BALANCE_INTERVAL))
        if last_tick <= 0 or current_tick > last_tick:
            try:
    ...
    async def _update_order_status(self):
        ...
        """
        if self._update_order_status_fetching:
            return
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            self._update_order_status_fetching = True
            ...
            for response, tracked_order in zip(responses, tracked_orders):
                ...
                if isinstance(response, SfoxAPIError):
    ...
    async def _trigger_order_fill(self,
                                  tracked_order: SfoxInFlightOrder,
    ...
    def tick(self, timestamp: float):
        ...
        """
        now = time.time()
        # Using 120 seconds here as SFOX websocket is quiet
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                channel: str = event_message.get("recipient", None)
                params: str = event_message.get("payload", None)

                if channel not in user_channels:
                    self.logger().error(f"Unexpected message in user stream: {event_message}.", exc_info=True)
                    continue
                if channel == Constants.WS_SUB["USER_ORDERS"]:
                    for trade_msg in params:
                        self._process_order_message(trade_msg)
                if channel == Constants.WS_SUB["USER_BALANCE"]:
        ...
        for order in result:
            ...
            ret_val.append(
                OpenOrder(
                    ...
                    trading_pair=convert_from_exchange_trading_pair(order["pair"]),
                    price=Decimal(str(order["price"])),
                    amount=Decimal(str(order["quantity"])),
                    executed_amount=Decimal(str(order["filled"])),
                    status=order["status"],
                    order_type=OrderType.LIMIT,
                    is_buy=True if order["o_action"].lower() == TradeType.BUY.name.lower() else False,
